            ...
            <v-col>
                <v-form>
                    <v-row>
                        ...
                        <v-col>
                            <v-text-field
                            v-model="searchValue" label="Search" @keyup.enter="searchProduct"/>
                        </v-col>
                        <v-col cols="auto">
        ...
        methods:{
            addCart(){
                    ...
                    const orderProductList = this.productList
                                .filter(product => product.selected && product.productCount > 0)
                                .map(product => ({productId: product.id, name:product.name, productCount: product.productCount}))
            ...
            scrollPaging(){
                // 현재화면높이 + 스크롤로 이동한거리 > 전체화면높이 - n(내가원하는 길이) 가 성립되면 추가 데이터 로드
            ...
            async loadData(){
                this.isLoading = true;
                // 문자열로 직접 ?size=xx&page=yy&productName=ee&&catogory=zz 형태로도 조립가능하지만, params라는 객체를 사용하면 파라이터 형식으로 url 조립
                let params={
                    size: this.pageSize,
            ...
            async createdOrder(){
                try {
                    const orderProductList = this.productList
                                .filter(product => product.selected && product.productCount > 0)
                                .map(product => ({productId: product.id,productCount: product.productCount}))
